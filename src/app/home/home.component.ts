...
export class HomeComponent {
    ...
    getPosts() {
       ...
       this.process = 0;
        ...
        console.log('');
        //this.webService.getapidata().subscribe(message1 => {
        //    this.apidata = message1;
    ...
    ngOnInit() {
        ...
        this.getPosts();
        this.getapidata();
        this.webService.getapidata().subscribe(message1 => { this.apidata = message1 })
        console.log(this.apidata)
        //this.getposts2();
    ...
    public getapidata() {
        this.webService.getapidata().subscribe(message1 => {
            ...
            for (var i = 0; i < this.apidata.length; i++) {
                ...
                if (this.apidata[i].Status == '200')

                    this.apidata[i].Status = 'Success'
                else
                    this.apidata[i].Status = 'stopped'
    ...
    public getsaperatedata() {
        this.success = []
        this.failed = []
        this.progress=[]
        for (var i = 0; i < this.data.length; i++) {
            if (this.data[i].Status == "Success") {
                this.success.push(this.data[i].Job_Name)
                
            }
            else if (this.data[i].Status == "Failed") {
                this.failed.push(this.data[i].Job_Name)
            }
            else {
                this.progress.push(this.data[i].Job_Name)
            }
        }
    }
}
